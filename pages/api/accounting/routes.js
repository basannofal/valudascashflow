...
export default async function handler(req, res) {
  if (req.method == "POST") {
    const { amount, collectedby, date, note, mid, bailmid, username } =
      req.body;
    try {
      // Query the database
      const q =
        "INSERT INTO `cf_borrow_payment`(`amount`, `date`, `note`, `m_id`, `bail_m_id`, `given_by`, `given_user`) VALUES (?, ?, ?, ?, ?, ?, ?)";

      const data = [amount, date, note, mid, bailmid, collectedby, username];
